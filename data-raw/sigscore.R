...
sigscore <- function(a, b,
  ...
  set.seed(seed)
  nes <- piano::runGSA(geneLevelStats = a[,1], geneSetStat = "gsea",
  ...
  nes.up <- c(nes$statDistinctDir[which(names(nes$gsc) == "UP"), 1],
              nes$Dir[which(names(nes$gsc) == "UP"), 1])
  nes.down <- c(nes$statDistinctDir[which(names(nes$gsc) == "DOWN"), 1],
  ...
  if (length(nes.up) == 0) {
    ...
    score <- c(sigscore = (nes.up[1] - nes.down[1])/2, p =  survcomp::combine.test(p = c(nes.up[2],
                                                                       nes.down[2]), method = "fisher", na.rm = TRUE))
  } else {
    score <- c(score = 0, p = 1)
  }
  return(score)
