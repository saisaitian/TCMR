...
group_list_all <- str_split(as.character(pdata$title), "_", simplify = T)[, 2]
head(group_list_all)
...
deg.cal <- function(i) {
  ...
  # limma，获取deg
  {
    # ~表示以~后面连接的东西作为因变量,level表示factor的排序，
    # 因为我们希望DMSO为0，实验组为1，只有这样limma包才能正确的匹配出来
    design <- model.matrix(~ factor(group_list, levels = group_list[c(3, 1)]))
